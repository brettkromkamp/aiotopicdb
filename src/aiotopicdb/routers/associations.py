...
async def get_association(map_id: int, association_id: str, scope: str = None, language: Language = None,
                          inline_resource_data: RetrievalMode = None,
                          resolve_attributes: RetrievalMode = None):
    association = await store.get_association(map_id, association_id, scope, language, inline_resource_data,
                                              resolve_attributes)
    if not association:
        raise HTTPException(status_code=404, detail="Association not found")
    result = AssociationModel.from_orm(association)
    return result
